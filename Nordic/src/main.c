...
void send_data_thread(void *p1, void *p2, void *p3)
{
	while (1) {
    	...
    	m.direction = cur_direction;
		printk("suunta= %d\nx = %d, y = %d, z = %d\n", m.direction, m.x, m.y, m.z);
    	my_lbs_send_sensor_notify(m.direction,m.x, m.y, m.z);
...
static void button_changed(uint32_t button_state, uint32_t has_changed)
{
	...
	if (button_state) {
            ...
            printk("Button pressed! New direction = %d\n", cur_direction);
...
int main(void)
{
	...
	//send_data_thread(NULL,NULL,NULL);
