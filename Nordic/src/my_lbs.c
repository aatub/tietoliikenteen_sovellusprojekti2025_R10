...
int my_lbs_send_sensor_notify(uint8_t direction, uint32_t sensor_valuex,uint32_t sensor_valuey,uint32_t sensor_valuez)
{
	if (!notify_mysensor_enabled) {
		return -EACCES;
	}
	char buf[32];
    snprintf(buf, sizeof(buf), "%u:%u:%u:%u",direction, sensor_valuex, sensor_valuey, sensor_valuez);
	bt_gatt_notify(NULL, &my_lbs_svc.attrs[7], buf, strlen(buf));
